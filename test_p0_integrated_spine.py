...
def test_p0_integrated_spine_recovers_and_records_evidence() -> None:
    ...
    assert "execution.failure" not in event_types
...
def test_failed_invocation_is_not_idempotency_cached_before_recovery() -> None:
    ...
    context = __import__("ois.kernel", fromlist=["ExecutionContext"]).ExecutionContext(
        identity=__import__("ois.kernel", fromlist=["ExecutionIdentity"]).ExecutionIdentity(),
