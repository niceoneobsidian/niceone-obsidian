...
def suggest_color(
    fg_rgb: tuple[float, float, float], bg_rgb: tuple[float, float, float], threshold: float = 1.75
...
def generate_from_template(
    ...
    for name, scheme in schemes.items():
        ...
        dest_path = (
            out_dir / f"{template_name}/{name}{template_ext}"
            if template_name != "vim"
            else out_dir / f"{template_name}/{name.replace(' ', '-')}{template_ext}"
        )
...
def generate_jsonl(out_dir: pathlib.Path, themes: dict[str, Theme]) -> None:
    with (out_dir / "schemes.jsonl").open("w", encoding="utf-8") as outf:
        for name, theme in sorted(themes.items()):
            data = json.dumps(
                theme, sort_keys=True, ensure_ascii=False, default=lambda o: o.to_dict()
            )
            outf.write(data + "\n")
...
def main() -> None:
    ...
    schemes = {}
    for iterm_scheme in rich.progress.track(
        ITERM_SCHEMES_PATH.glob("*.itermcolors"), description="Reading iTerm schemes"
    ):
    ...
    for yml_scheme in rich.progress.track(
        YAML_SCHEMES_PATH.glob("*.yml"), description="Reading YAML schemes"
    ):
