...
def suggest_color(
    fg_rgb: tuple[float, float, float], bg_rgb: tuple[float, float, float], threshold: float
...
def check_scheme(
    plist: dict, scheme_name: str, threshold: float
) -> tuple[
    bool,
    list[
        tuple[
    ...
    for i in range(1, 15):
        ...
        if key in plist:
            ...
            passed = passes_wcag(ratio, threshold)
            suggested_fg = (
                suggest_color(ansi_rgb, background_rgb, threshold) if not passed else None
            )
    ...
    if cursor_text_key in plist and cursor_key in plist:
        ...
        passed = passes_wcag(ratio, threshold)
        suggested_fg = (
            suggest_color(cursor_text_rgb, cursor_bg_rgb, threshold) if not passed else None
        )
        checks.append(
            ("Cursor Text on Cursor", passed, ratio, cursor_text_rgb, cursor_bg_rgb, suggested_fg)
        )
    else:
    ...
    if sel_bg_key in plist and sel_text_key in plist:
        ...
        checks.append(
            ("Selection (text on bg)", passed, ratio, sel_text_rgb, sel_bg_rgb, suggested_fg)
        )
    else:
...
def generate_html(
    total_failing: int, total_schemes: int, failing_schemes: list, threshold: float
) -> str:
...
"""
    if total_failing == 0:
        html += '<p style="font-size: 1.1em; color: green;">All schemes pass! 🎉</p>'
    else:
...
def resolve_iterm_files(schemes_dir: Path, scheme_names: list[str] | None) -> list[Path]:
    if scheme_names:
        files: list[Path] = []
        missing: list[str] = []
...
def _format_check_line(
    ...
    passed: bool,
    ratio: float,
    fg_rgb: tuple[float, float, float] | None,
    bg_rgb: tuple[float, float, float] | None,
    suggested_fg: tuple[float, float, float] | None,
    threshold: float,
) -> str:
    reset = "\033[0m"
    red = "\033[31m"
    ...
    if fg_rgb is None or bg_rgb is None:
        ...
        lines.append(
            f"  - {color_demo} {check_name}: {color}{status}{reset} (needs >= {threshold}:1)"
        )
...
def print_single_scheme_results(name: str, checks: list, all_pass: bool, threshold: float) -> None:
    reset = "\033[0m"
    green = "\033[32m"
    red = "\033[31m"
