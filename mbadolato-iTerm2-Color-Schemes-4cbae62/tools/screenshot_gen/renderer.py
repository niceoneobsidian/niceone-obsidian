...
class PixFont:
    def __init__(self, font_img, charset):
        ...
        assert not remainder, (
            f"Font image width {font_img.width} is not divisible by {len(font_charset)}"
        )
        self.char_width = char_width
        self.char_height = font_img.height
        self.char_images = {}
        for i, c in enumerate(font_charset):
            self.char_images[c] = font_img.crop(
                (i * char_width, 0, (i + 1) * char_width, self.char_height)
            )
...
class ConsoleScreenshotRenderer:
    ...
    def __init__(self, pix_font: PixFont, scheme):
        ...
        self.rgb_colors = {
            name: (col["r"], col["g"], col["b"])
            for name, col in scheme.items()
        ...
        for i in range(9):
            ...
            for bright_bit in (0, 1):
                ...
                for bg_color in range(9):
                    ...
                    if color < 0:
                        pil_color = (
                            self.rgb_colors["Bold_Color"]
                            if bright_bit
                            else self.rgb_colors["Foreground_Color"]
                        )
                    else:
    ...
    def _draw_cursor_color(self):
        ...
        cursor_text_color = (
            self.rgb_colors.get("Cursor_Text_Color") or self.rgb_colors["Background_Color"]
        )
    ...
    def _draw_selection_color(self):
        ...
        selection_text_color = (
            self.rgb_colors.get("Selected_Text_Color") or self.rgb_colors["Foreground_Color"]
        )
