...
def save_scaling_tests(img, dest_path_base):
    ...
    for resampling in Image.Resampling:
        ...
        img2.save(
            dest_path_base.with_suffix(f".{resampling.name}.png"), optimize=True, compress_level=9
        )
...
def main():
    ...
    ap.add_argument(
        "--scaling-test-mode", action="store_true", help="Generate images for scaling tests"
    )
