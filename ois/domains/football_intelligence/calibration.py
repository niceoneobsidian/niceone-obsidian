"""Probability evaluation primitives for football models."""
...
def evaluate(probabilities: Sequence[Sequence[float]], outcomes: Sequence[int]) -> CalibrationMetrics:
    ...
    return CalibrationMetrics(
        brier_score=multiclass_brier(probabilities, outcomes),
        log_loss=multiclass_log_loss(probabilities, outcomes),
        accuracy=sum(
            max(range(3), key=probabilities[i].__getitem__) == outcomes[i]
