"""Governed football model federation with abstention and simulation."""
...
class FootballEnsemble:
    """Reference v1 ensemble; learned weights require evaluation before promotion."""
    ...
    def predict(self, match: MatchState, *, simulation_seed: int | None = None) -> FootballPrediction:
        ...
        agreement = max(0.0, min(1.0, 1.0 - pstdev([m.home for m in models]) / 0.25))
        ...
        simulation = simulate_match(expected_home, expected_away, iterations=self.simulation_iterations, correlation=0.08, seed=simulation_seed)
