"""Deterministic football feature engineering primitives.

Features are computed only from the supplied pre-match MatchState. The module
intentionally contains no future-match access, making it safe as a foundation
for walk-forward training pipelines.
"""
...
def build_feature_vector(match: MatchState) -> dict[str, float]:
    """Return a stable, numeric pre-match feature vector."""
...
def sigmoid(value: float) -> float:
    """Numerically stable sigmoid for future lightweight scoring models."""
    if value >= 0:
        z = exp(-value)
        return 1.0 / (1.0 + z)
    z = exp(value)
