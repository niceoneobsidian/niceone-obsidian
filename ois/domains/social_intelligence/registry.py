...
class SociaVaultTool:
    ...
    def invoke(self, request: InvocationRequest) -> InvocationResult:
        try:
            ...
            return InvocationResult(request.invocation_id, self.contract.capability_id,
                                    InvocationStatus.SUCCEEDED, output=output)
...
def build_social_tool_registry() -> ToolRegistry:
    ...
    registry.register("tool.social.sociavault", SociaVaultTool(SociaVaultAdapter()))
    registry.register("tool.social.bundle_social", BundleSocialTool(BundleSocialAdapter()))
    return registry
...
def provider_capability_contracts() -> tuple[CapabilityContract, ...]:
    return (
        CapabilityContract(
            ...
            input_schema={"type": "object", "required": ["handle"]},
            output_schema={"$ref": "SocialProfile"},
            permissions=("social.read",), allowed_domains=("social_intelligence",),
        ),
        CapabilityContract(
        ...
        CapabilityContract(
            ...
            input_schema={"$ref": "SocialPublishRequest"}, output_schema={"$ref": "SocialPublishResult"},
            risk_level=RiskLevel.HIGH, permissions=("social.publish",),
            allowed_domains=("social_growth", "social_intelligence"),
            side_effects=SideEffectLevel.EXTERNAL, idempotent=False,
        ),
        CapabilityContract(
            capability_id="social.analytics.bundle_social.post",
            version="1.0.0",
            description="Retrieve normalized post analytics through bundle.social.",
            input_schema={"type": "object", "required": ["post_id", "platform"]},
            output_schema={"$ref": "SocialAnalytics"}, permissions=("social.analytics",),
            allowed_domains=("social_intelligence",),
        ),
    )
