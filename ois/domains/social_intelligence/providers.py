...
class SociaVaultAdapter:
    ...
    def __init__(self, timeout: float = 30.0) -> None:
        self._client = _HttpClient(self.base_url, "SOCIAVAULT_API_KEY", timeout)

    def tiktok_profile(self, handle: str) -> SocialProfile:
        payload = self._client._request("GET", "/scrape/tiktok/profile", params={"handle": handle})
    ...
    def tiktok_search(self, query: str, kind: str = "keyword") -> SocialSearchResult:
        allowed = {"users": "/scrape/tiktok/search/users", "hashtag": "/scrape/tiktok/search/hashtag",
                   "keyword": "/scrape/tiktok/search/keyword", "top": "/scrape/tiktok/search/top"}
        path = allowed.get(kind)
        if path is None:
            raise ValueError(f"unsupported TikTok search kind: {kind}")
        payload = self._client._request("GET", path, params={"query": query})
        data = payload.get("data", payload)
        ...
        for item in items:
            if not isinstance(item, dict):
                continue
            if kind == "users":
                profiles.append(SocialProfile(provider=self.provider_id, platform="tiktok",
                    external_id=str(item.get("id")) if item.get("id") is not None else None,
                    handle=item.get("username") or item.get("unique_id"),
                    display_name=item.get("nickname") or item.get("display_name"),
                    followers=item.get("followers") or item.get("follower_count"), raw=item))
            else:
                posts.append(SocialPost(provider=self.provider_id, platform="tiktok",
                    ...
                    url=item.get("url") or item.get("video_url"), media_type="video", raw=item,
                    observed_at=datetime.now(UTC)))
        return SocialSearchResult(provider=self.provider_id, platform="tiktok", query=query,
                                  profiles=profiles, posts=posts, raw=payload,
                                  observed_at=datetime.now(UTC))
...
class BundleSocialAdapter:
    ...
    def __init__(self, timeout: float = 30.0) -> None:
        self._client = _HttpClient(self.base_url, "BUNDLE_SOCIAL_API_KEY", timeout)

    def create_post(self, request: SocialPublishRequest) -> SocialPublishResult:
        payload = self._client._request("POST", "/post", body={
        ...
        return SocialPublishResult(provider=self.provider_id,
            external_post_id=str(data.get("id")) if data.get("id") is not None else None,
            status=str(data.get("status", "UNKNOWN")),
            platform_results=data.get("platforms", data.get("results", {})) or {}, raw=payload,
            observed_at=datetime.now(UTC))

    def post_analytics(self, post_id: str, platform: str) -> SocialAnalytics:
        payload = self._client._request("GET", "/analytics/post",
                                        params={"postId": post_id, "platformType": platform})
        data = payload.get("data", payload)
        if not isinstance(data, dict):
            raise ProviderError("invalid bundle.social analytics response")
        metrics = data.get("metrics", data)
        return SocialAnalytics(provider=self.provider_id, platform=platform,
            external_post_id=post_id, metrics=metrics if isinstance(metrics, dict) else {},
            raw=payload, observed_at=datetime.now(UTC))
