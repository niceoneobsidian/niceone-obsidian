...
SOCIAL_CAPABILITIES: tuple[DomainCapability, ...] = (
    ...
    DomainCapability("social.publish.approve", "Request/resolve publishing approval", "PublishIntent", "ApprovalDecision", requires_approval=True),
    DomainCapability("social.publish", "Publish through a registered connector", "PublishIntent", "ExternalActionResult", side_effect=True, requires_approval=True),
    DomainCapability("social.analytics.collect", "Collect platform performance", "PublicationRef", "PerformanceSnapshot"),
    DomainCapability("social.learning.update", "Update social learning memory", "PerformanceSnapshot", "LearningUpdate"),
)
