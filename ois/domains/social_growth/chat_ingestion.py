...
def ingest_chat_observations(
    ...
    # Preserve the append result for each event. Metrics are materialized
    # only when the event was newly inserted, not for duplicates.
    ...
    if analytics_store is not None:
        for event, was_added in zip(events, append_results, strict=True):
            if was_added:
