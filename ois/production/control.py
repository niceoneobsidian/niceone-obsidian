...
class DeploymentController:
    ...
    def deploy(self, version: str, environment: str, *, approved_by: str | None = None, evidence: tuple[str, ...] = ()) -> DeploymentRecord:
        if not approved_by:
    ...
    def rollback(self, environment: str, *, approved_by: str | None = None, evidence: tuple[str, ...] = ()) -> DeploymentRecord:
        if not approved_by:
...
class DistributedCoordinator:
    ...
    def claim(self, job_id: str, worker_id: str, ttl_seconds: float = 30.0) -> Lease | None:
        now = time.time()
        row = self._db.execute("SELECT worker_id, expires FROM leases WHERE job_id = ?", (job_id,)).fetchone()
        if row and row[1] > now and row[0] != worker_id:
            return None
        expires = now + ttl_seconds
        self._db.execute("INSERT OR REPLACE INTO leases VALUES (?, ?, ?)", (job_id, worker_id, expires))
        self._db.execute("INSERT OR IGNORE INTO jobs(job_id, state) VALUES (?, 'claimed')", (job_id,))
        self._db.commit()
    ...
    def complete(self, job_id: str, worker_id: str, result: Any) -> None:
        ...
        self._db.execute("UPDATE jobs SET state = 'completed', result = ? WHERE job_id = ?", (json.dumps(result, sort_keys=True), job_id))
...
class SemanticWorld:
    ...
    def __init__(self, database: str = ":memory:") -> None:
        self._db = sqlite3.connect(database)
        self._db.execute("CREATE TABLE IF NOT EXISTS entities (id TEXT PRIMARY KEY, type TEXT NOT NULL, data TEXT NOT NULL)")
        self._db.execute(
    ...
    def upsert_entity(self, entity_id: str, entity_type: str, data: dict[str, Any]) -> None:
        self._db.execute("INSERT OR REPLACE INTO entities VALUES (?, ?, ?)", (entity_id, entity_type, json.dumps(data, sort_keys=True)))
        self._db.commit()

    def add_fact(self, subject: str, predicate: str, obj: str, source: str) -> int:
        row = self._db.execute(
            "SELECT COALESCE(MAX(version), 0) FROM facts WHERE subject = ? AND predicate = ?", (subject, predicate)
        ).fetchone()
        version = int(row[0]) + 1
        cursor = self._db.execute(
            "INSERT INTO facts(subject,predicate,object,source,version) VALUES(?,?,?,?,?)", (subject, predicate, obj, source, version)
        )
        self._db.commit()
    ...
    def facts(self, subject: str) -> tuple[dict[str, Any], ...]:
        rows = self._db.execute(
            "SELECT subject,predicate,object,source,version FROM facts WHERE subject = ? ORDER BY id", (subject,)
        ).fetchall()
        return tuple({"subject": r[0], "predicate": r[1], "object": r[2], "source": r[3], "version": r[4]} for r in rows)
