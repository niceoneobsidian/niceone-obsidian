...
class PostgresExecutionStore:
    ...
    def stage_side_effect(
        ...
        def _execute(conn: Connection) -> None:
            ...
            cursor.execute(
                ...
                "(effect_id, tenant_id, execution_id, invocation_id, capability_id, "
    ...
    def get_outbox_effect(self, effect_id: str) -> dict[str, Any] | None:
        with self.transaction() as connection:
            cursor = connection.cursor()
            cursor.execute(
                "SELECT effect_id, status, request, result "
    ...
    def claim_and_stage_outbox(
        ...
        """Atomically claim invocation and stage outbox entry.

        Returns True if claimed and staged; False if already claimed (no outbox staged).
        Fails closed with ValueError on fingerprint reuse.
        """
        with self.transaction() as connection:
            claimed = self.claim_idempotency(
