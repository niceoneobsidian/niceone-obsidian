...
def test_transient_retry_boundary_zero_retries_escalates_immediately():
...
def test_transient_retry_boundary_one_retry_allows_exactly_one():
...
def test_retry_count_and_recovery_attempts_are_independent_counters():
...
def test_stop_is_reached_even_on_first_safety_failure():
...
def test_permission_failure_escalates_but_is_not_marked_terminal():
...
def test_negative_max_recovery_attempts_rejected():
    with pytest.raises(ValueError):
