...
def test_structural_fabric_capability_executes_through_kernel() -> None:
    ...
    fabric_runtime.bind_workflow(
        WorkflowSpec("social.workflow", nodes=("collect", "analyze"), edges=(("collect", "analyze"),))
    )
    ...
    context = ExecutionContext(
        identity=ExecutionIdentity(workflow_id="social.workflow", workflow_version="1.0.0"),
    ...
    result = runtime.execute(
        ...
        "fabric.workflow",
        "1.0.0",
        {"workflow_id": "social.workflow", "execution_id": str(context.identity.execution_id), "inputs": {"topic": "AI"}},
    ...
    assert any(event.event_type == "execution.checkpointed" for event in evidence.list(context.identity.execution_id))
