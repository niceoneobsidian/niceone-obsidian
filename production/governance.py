...
class DistributedCoordinator:
    ...
    def __init__(self, database: str = ":memory:") -> None:
        ...
        self._db.execute(
            ...
            "job_id TEXT PRIMARY KEY, state TEXT NOT NULL, result TEXT)"
        )
        self._db.execute(
            "CREATE TABLE IF NOT EXISTS leases ("
            "job_id TEXT PRIMARY KEY, worker_id TEXT NOT NULL, expires REAL NOT NULL)"
        )
        self._db.commit()
...
class SemanticWorld:
    ...
    def add_fact(self, subject: str, predicate: str, obj: str, source: str) -> int:
        ...
        if cursor.lastrowid is None:
            raise RuntimeError("Failed to retrieve inserted fact ID")
        return int(cursor.lastrowid)

    def facts(self, subject: str) -> tuple[dict[str, Any], ...]:
        rows = self._db.execute(
            (
                "SELECT subject,predicate,object,source,version "
                "FROM facts WHERE subject = ? ORDER BY id"
            ),
            (subject,),
        ).fetchall()
        return tuple(
