...
def test_tiktok_page_enters_gateway_before_cursor_advances():
    ...
    def opener(request: Request, timeout: float):
        ...
        return FakeResponse({"data": {"videos": [{"id": "123"}], "cursor": 1, "has_more": True}, "error": {"code": "ok"}})
    ...
    result = source.ingest_pages(tenant_id="tenant", workspace_id="workspace", credential_id="unused", max_pages=1)
...
def test_live_tiktok_display_source():
    ...
    result = source.ingest_pages(tenant_id="live-test", workspace_id="live-test", credential_id="external", max_pages=1)
    assert result.pages == 1
