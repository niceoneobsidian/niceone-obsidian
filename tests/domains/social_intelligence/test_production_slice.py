...
def test_tiktok_normalization_is_deterministic():
    raw = {"data": {"videos": [{
        "id": "v1",
...
def test_readiness_manifest_cannot_claim_production_without_evidence():
    manifest = build_readiness_manifest()
    assert manifest.production_ready is False
    assert all(check.status != "production_verified" for check in manifest.checks)


def test_scope_and_hash_contracts():
    payload = {"data": {"videos": [{"id": "v1"}]}}
    evidence = RawEvidence(
        "e1", "tenant", "workspace", "tiktok.display.v2", "cursor:initial",
        payload, canonical_hash(payload), datetime.now(UTC), "tiktok-display-v2", "tiktok.display.v2", "run-1",
    )
    event = OutboxEvent(
        "evt-1", "tenant", "workspace", "source.raw_evidence.created", "e1",
        {"evidence_id": "e1", "source_id": "tiktok.display.v2", "payload_hash": evidence.payload_hash},
        datetime.now(UTC),
    )
