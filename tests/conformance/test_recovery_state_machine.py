...
def test_retry_decision_enters_retry_pending():
    ...
    assert RecoveryPlane().apply(ExecutionState.VALIDATING, decision) == ExecutionState.RETRY_PENDING


def test_exhausted_recovery_escalates():
    decision = RecoveryPlane().decide(failure="timeout", attempt=3, max_attempts=3)
    assert decision.action == "escalate"
    assert RecoveryPlane().apply(ExecutionState.VALIDATING, decision) == ExecutionState.ESCALATION_REQUIRED
