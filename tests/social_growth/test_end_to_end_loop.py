...
from ois.domains.social_growth.experiments import DeterministicExperimentExecutor, ExperimentRegistry
...
def test_research_publish_measure_attribute_learn_loop() -> None:
    ...
    connectors.register(
        GenericSocialConnector(
            "tiktok", publish_callable=lambda intent: published.append(intent.content) or {"id": "pub-1"}
        )
    )
    ...
    research = runtime.execute(
        context, "social.research.execute", "1.0.0",
        {"query": "AI content", "events": [{
            "platform": "tiktok", "event_type": "post",
            "occurred_at": "2026-08-23T00:00:00+00:00", "external_id": "p1",
            "text": "AI content is accelerating", "metrics": {"likes": 100, "comments": 10},
        }]},
    ...
    intent = PublishIntent(
        platform="tiktok", account_ref="test-account", content={"text": "AI content"},
    )
    context.approvals.append({"intent_id": intent.intent_id, "approved": True, "actor": "test"})
    publish = runtime.execute(
        context, "social.publish", "1.0.0", {"intent": intent.model_dump(mode="json")},
    ...
    analytics.append(MetricObservation(
        observation_id="obs-1", entity_id="pub-1", metric="engagement",
        value=0.12, observed_at=datetime.now(UTC), platform="tiktok",
    ))
    assert analytics.latest("pub-1", "engagement") is not None

    attribution = linear_attribution(
        "conversion-1", [AttributionTouchpoint("pub-1", "campaign-1", "publish", datetime.now(UTC))], 100.0
    )
    ...
    experiment = ExperimentSpec(
        experiment_id="exp-1", hypothesis="higher engagement", metric="engagement",
        control={"text": "A"}, variants=[{"text": "B"}], success_threshold=0.1,
    )
