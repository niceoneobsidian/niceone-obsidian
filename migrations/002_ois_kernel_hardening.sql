...
DROP POLICY IF EXISTS tenant_state_write ON ois_graph_state_store;
CREATE POLICY tenant_state_select ON ois_graph_state_store FOR SELECT
USING (tenant_id = NULLIF(current_setting('app.current_tenant_id', true),'')::uuid);
CREATE POLICY tenant_state_write ON ois_graph_state_store FOR INSERT
...
BEGIN RAISE EXCEPTION 'OIS immutable ledger: UPDATE/DELETE forbidden'; END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS lock_evidence_ledger ON ois_attestation_evidence;
CREATE TRIGGER lock_evidence_ledger BEFORE UPDATE OR DELETE ON ois_attestation_evidence
FOR EACH ROW EXECUTE FUNCTION ois_prevent_mutation();
DROP TRIGGER IF EXISTS lock_dlq_ledger ON ois_kernel_dead_letter_queue;
CREATE TRIGGER lock_dlq_ledger BEFORE UPDATE OR DELETE ON ois_kernel_dead_letter_queue
FOR EACH ROW EXECUTE FUNCTION ois_prevent_mutation();
DROP TRIGGER IF EXISTS lock_registry_ledger ON ois_artifact_registry;
CREATE TRIGGER lock_registry_ledger BEFORE UPDATE OR DELETE ON ois_artifact_registry
FOR EACH ROW EXECUTE FUNCTION ois_prevent_mutation();
