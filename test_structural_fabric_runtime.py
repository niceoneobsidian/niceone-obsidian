...
def test_workflow_worker_and_agent_runtime_bindings() -> None:
    ...
    runtime.bind_workflow(WorkflowSpec("wf", nodes=("research", "write"), edges=(("research", "write"),)))
    runtime.bind_worker(WorkerSpec("worker", queue="default", concurrency=2, max_attempts=2))
...
def test_llm_gateway_routes_to_registered_provider() -> None:
    runtime = FabricRuntime()
    runtime.model_router.register(ModelRoute("route", provider="test", model="model-a", capabilities=("chat",)))
    gateway = runtime.configure_gateway(LLMGatewaySpec("gateway", providers=("test",)))
    gateway.register_provider("test", lambda model, payload: {"model": model, "text": payload["prompt"]})
...
def test_context_knowledge_reasoning_social_and_learning() -> None:
    runtime = FabricRuntime()
    runtime.bind_knowledge(KnowledgeArtifact("doc-1", source_ref="source", content_ref="OIS social intelligence", metadata={"topic": "social"}))
    runtime.context.knowledge["doc-1"] = {"title": "OIS"}
    runtime.bind_reasoning(ReasoningPattern("reflection", objective_classes=("analysis",), topology="loop"))
    runtime.bind_social(SocialSignal("sig-1", platform="tiktok", topic_refs=("ai",), signal_type="mention", value=12, confidence=0.9))

    context = runtime.bind_context(ContextRequest("ctx", objective="analyze", knowledge_refs=("doc-1",)))
    ...
    runtime.bind_learning(LearningCandidate("candidate-1", hypothesis="improve hook", evidence_refs=("eval-1",)))
    promoted = runtime.learning.promote("candidate-1", approved=True, evidence_refs=("approval-1",))
