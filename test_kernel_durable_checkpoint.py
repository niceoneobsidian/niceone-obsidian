...
def test_json_checkpoint_store_persists_execution(tmp_path):
    ...
    assert restored.working_memory["answer"] == {
        "value": 42
    }
...
def test_json_checkpoint_survives_store_recreation(tmp_path):
    ...
    first_store = JsonFileCheckpointStore(
        str(checkpoint_path)
    )
    ...
    second_store = JsonFileCheckpointStore(
        str(checkpoint_path)
    )

    restored = second_store.load(
        context.identity.execution_id
    )

    assert (
        restored.working_memory["survives_restart"]
        is True
    )
