...
class FakeEngine:
    ...
    def acquire(self, request):
        self.calls += 1
        return SourceDocument(request.url, 200, "text/html", self.body, self.name, "now")


def test_route_learner_prefers_successful_engine():
...
def test_capability_extracts_validates_and_records_provenance():
...
def test_capability_falls_back_after_engine_failure():
...
def test_http_engine_refuses_javascript_route():
    engine = HttpAcquisitionEngine()
    assert engine.can_handle(WebIntelligenceRequest("https://example.com"))
    assert not engine.can_handle(WebIntelligenceRequest("https://example.com", javascript_required=True))


def test_validator_rejects_empty_source():
    validator = WebIntelligenceValidator()
    source = SourceDocument("https://example.com", 200, "text/html", "", "fake", "now")
